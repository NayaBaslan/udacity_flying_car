...
//  cmd.desiredThrustsN[3] = mass * 9.81f / 4.f; // rear right
  float length_perp = L / sqrtf(2);
  float pb = momentCmd.x / length_perp;
  float qb = momentCmd.y / length_perp;
  float rb = momentCmd.z * (-1) / kappa;

  cmd.desiredThrustsN[0] = ( pb + qb + rb + collThrustCmd)/4;  
  cmd.desiredThrustsN[1] = (-pb + qb - rb + collThrustCmd)/4;
  cmd.desiredThrustsN[2] = ( pb - qb - rb + collThrustCmd)/4; 
  cmd.desiredThrustsN[3] = (-pb - qb + rb + collThrustCmd)/4;
...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  // Implementing the equations
     float b_X = CONSTRAIN(accelCmd.x/accel_coll, -maxTiltAngle, maxTiltAngle);
     float b_Xp = kpBank * (b_X - R(0,2));

     float b_Y = CONSTRAIN(accelCmd.y/accel_coll, -maxTiltAngle, maxTiltAngle);
     float b_Yp = kpBank * (b_Y - R(1,2));

     pqrCmd.x = 1 / R(2,2) * ( R(1,0) * b_X - R(0,0) * b_Yp) ;
     pqrCmd.y = 1 / R(2,2) * ( R(1,1) * b_Xp - R(0,1) * b_Yp) ;
  }
  /////////////////////////////// END STUDENT CODE ////////////////////////////
...
V3F QuadControl::LateralPositionControl(V3F posCmd, V3F velCmd, V3F pos, V3F vel, V3F accelCmdFF)
{
  ...
  accelCmd= kpPosXY * (error_pos) + kpVelXY * (error_vel) + accelCmd;
...
float QuadControl::YawControl(float yawCmd, float yaw)
{
  ...
  float error_yaw = yawCmd_float - yaw;
