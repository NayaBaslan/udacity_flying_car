...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
 ...
 float predictedPitch = pitchEst + dtIMU * gyro_R.y;
 float predictedRoll = rollEst + dtIMU * gyro_R.x;
 ekfState(6) = ekfState(6) + dtIMU * gyro_R.z;	// yaw
 
//replace with MAT3x3F 
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(5) = curState(5) + (accel_inertial.z -9.81)*dt;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  zFromX(0) = ekfState(6);
  float diff_yaw = magYaw - zFromX(0);
  if (diff_yaw - 3.14 >0)  
    zFromX(0) = zFromX(0) + 2.0*3.14;
  else if (diff_yaw + 3.14 < 0)
    zFromX(0) = zFromX(0) - 2.0*3.14;
